...
import { Folder, ChevronRight, X, ArrowLeft } from "lucide-react";
...
export default function MobileFolderPickerModal({
  ...
  const loadDirectory = async (path: string) => {
    ...
    setErrorMsg("");
    try {
      const dirs: DirEntry[] = await invoke("list_directories", { path });
      setDirectories(dirs);
    } catch (err) {
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
          {loading ? (
            ...
            <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-sub)' }}>该目录下没有文件夹</div>
          ) : (
            directories.map((dir) => (
              <div 
                key={dir.path}
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: '1rem' }}>{dir.name}</span>
        ...
        <div className="modal-footer" style={{ padding: '16px', borderTop: '1px solid var(--border)' }}>
          <button 
            ...
            style={{ width: '100%', padding: '14px', fontSize: '1.1rem', borderRadius: '12px', background: 'var(--accent)', color: '#fff', border: 'none', fontWeight: 'bold' }}
